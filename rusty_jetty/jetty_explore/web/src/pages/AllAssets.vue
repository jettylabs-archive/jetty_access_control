<template>
  <q-page class="flex column container-md">
    <JettyTable title="All Assets" :rows-per-page="30" :filter-method="filterMethod" :columns="columns"
      :csv-config="csvConfig" fetchPath="/api/assets" v-slot="slotProps">
      <q-tr>
        <q-td key="name">
          <router-link :to="'/asset/' + encodeURIComponent(slotProps.props.row.name)"
